...
OMX_ERRORTYPE OSAL_QueueCreate(OSAL_QUEUE *queueHandle, int maxNumElem)
{
    ...
    queue->first = (OSAL_QElem *)OSAL_Malloc(sizeof(OSAL_QElem));
    if (queue->first == NULL)
        return OMX_ErrorInsufficientResources;

    OSAL_Memset(queue->first, 0, sizeof(OSAL_QElem));
    currentqelem = queue->last = queue->first;
    queue->numElem = 0;
    queue->maxNumElem = maxNumElem;
    for (i = 0; i < (queue->maxNumElem - 2); i++) {
        newqelem = (OSAL_QElem *)OSAL_Malloc(sizeof(OSAL_QElem));
        if (newqelem == NULL) {
            while (queue->first != NULL) {
                currentqelem = queue->first->qNext;
                OSAL_Free((OMX_PTR)queue->first);
                queue->first = currentqelem;
            }
            return OMX_ErrorInsufficientResources;
        } else {
            OSAL_Memset(newqelem, 0, sizeof(OSAL_QElem));
